...
export default function Header() {
    ...
    for (let i = 0; i < stationsList.length; i++) {
      ...
      if (currentStation !== null) {
        ...
        let response = await fetch(
          "https://dev-amzdsp-api.dispatchex.com/api/RouteDemand/AddRouteDemand",
          {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
              ...
              routeDemandDetail: [...currentStation?.routes],
            }),
          }
        );
  ...
  return (
    <>
      <div className="header row bg-secondary px-sm-4 px-2 py-3">
        <div className="d-flex justify-content-between align-items-center">
          <h3 className="p-0 m-0 text-white">Add Demand</h3>
          <div className="btns">
            <NavLink
              to="/view-demands"
              className="btn btn-warning rounded-3 px-4 py-2 me-sm-3 me-0 mb-sm-0 mb-3 d-sm-inline d-block"
            >
              View Demands
            </NavLink>
            <button
              className="btn btn-warning rounded-3 px-4 py-2 d-sm-inline d-block"
              disabled={stationsList.length > 0 ? false : true}
              onClick={submitDemands}
            >
              Submit
            </button>
          </div>
      ...
      {demandSuccess && (
        <div
          className="alert alert-success mt-4 mx-auto col-md-5 col-11 d-flex justify-content-center"
          role="alert"
        >
